...
function keysFromEnv(): { privateKey?: crypto.KeyObject; publicKey?: crypto.KeyObject; publicKeyPem?: string } {
  ...
  if (!priv || !pub) return {};
  try { return { privateKey: crypto.createPrivateKey(priv), publicKey: crypto.createPublicKey(pub), publicKeyPem: pub }; }
  catch { return {}; }
}

function issueAuthorization(intent: IntentRecord, policy: PolicyResult): AuthorizationEnvelope {
  const envKeys = keysFromEnv();
  const pair = envKeys.privateKey && envKeys.publicKey ? envKeys : crypto.generateKeyPairSync('ed25519');
  const publicKeyPem = envKeys.publicKeyPem ?? pair.publicKey!.export({ type: 'spki', format: 'pem' }).toString();
  const issuedAt = new Date().toISOString();
  const expiresAt = new Date(Date.now() + 15 * 60_000).toISOString();
  const base = { id: `auth_${crypto.randomUUID()}`, intentId: intent.id, workspaceId: intent.workspaceId, policyVersion: policy.policyVersion, policyHash: policy.policyHash, scope: ['lead:read', 'whatsapp:template:lead-recovery'], authorizedAction: 'WHATSAPP_TEMPLATE' as const, targetRef: intent.lead.leadRef, issuedAt, expiresAt, nonce: crypto.randomBytes(24).toString('hex') };
...
function verifyAuthorization(auth: AuthorizationEnvelope): boolean {
  ...
  try { const { signature, publicKey, ...base } = auth; return crypto.verify(null, Buffer.from(canonical(base)), crypto.createPublicKey(publicKey), Buffer.from(signature, 'base64url')); }
  catch { return false; }
}
...
export function ingestLead(input: { workspaceId: string; leadRef: string; consent: boolean; source?: 'META_LEAD_ADS' }): IntentRecord {
  ...
  const policy = policyFor(lead); intent.policy = policy;
  if (policy.verdict === 'DENY') intent.status = 'POLICY_BLOCKED';
  else { intent.authorization = issueAuthorization(intent, policy); if (!verifyAuthorization(intent.authorization)) throw new Error('AUTHORIZATION_VERIFICATION_FAILED'); intent.status = 'AUTHORIZED'; }
  intent.updatedAt = new Date().toISOString(); store.set(intent.id, intent); return intent;
}

export function propagate(intentId: string, mode: ExecutionMode): IntentRecord {
  const intent = store.get(intentId); if (!intent) throw new Error('INTENT_NOT_FOUND');
  ...
  intent.propagationId = `prop_${crypto.randomUUID()}`; intent.status = 'PROPAGATED'; intent.updatedAt = new Date().toISOString(); store.set(intent.id, intent); return intent;
}

export function observe(intentId: string, status: ObservationStatus): IntentRecord {
  const intent = store.get(intentId); if (!intent) throw new Error('INTENT_NOT_FOUND');
  intent.observation = status; intent.status = status === 'UNKNOWN' ? 'OBSERVED' : 'COMPLETED'; intent.updatedAt = new Date().toISOString(); store.set(intent.id, intent); return intent;
}

export function getIntent(id: string): IntentRecord | undefined { return store.get(id); }
export function summary(workspaceId: string) { const records = [...store.values()].filter(x => x.workspaceId === workspaceId); const completed = records.filter(x => x.observation === 'QUALIFIED' || x.observation === 'OPPORTUNITY').length; return { intents: records.length, authorized: records.filter(x => x.status !== 'POLICY_BLOCKED').length, propagated: records.filter(x => Boolean(x.propagationId)).length, qualified: completed, policyBlocks: records.filter(x => x.status === 'POLICY_BLOCKED').length, records }; }
