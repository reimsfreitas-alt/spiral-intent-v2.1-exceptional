...
test('CONFIRMED exact match with corroboration',()=>expect(verifyEffect(a,pair(50000)).verdict).toBe('CONFIRMED'));
test('DEVIATED amount mismatch',()=>expect(verifyEffect(a,pair(5000000)).verdict).toBe('DEVIATED'));
test('INCONCLUSIVE external failure',()=>expect(verifyEffect(a,pair(50000,50000,'failed')).verdict).toBe('INCONCLUSIVE'));
test('INCONCLUSIVE scope mismatch',()=>expect(verifyEffect(a,[{...o(50000,'succeeded','obs_1'),operation:'charge'},o(50000,'succeeded','obs_2')] as any).verdict).toBe('INCONCLUSIVE'));
test('INCONCLUSIVE single observation',()=>expect(verifyEffect(a,[o(50000)]).verdict).toBe('INCONCLUSIVE'));
test('CONFLICTED observer disagreement',()=>expect(verifyEffect(a,pair(50000,5000000)).verdict).toBe('CONFLICTED'));
test('deterministic',()=>expect(verifyEffect(a,pair(5000000))).toEqual(verifyEffect(a,pair(5000000))));
