...
export interface Execution { execution_id: string; intent_id: string; external_operation_id: string; requested_amount: number; executed_amount: number; deviate: boolean; is_simulated_deviation?: boolean; warning?: string; created_at: string; }
export interface Observation { system: 'stripe'; operation: 'refund'; external_operation_id: string; amount: number; currency: string; status: string; observed_at: string; observer_id: string; trust_domain: { org: string; cloud: string; credential_class: string; }; freshness_ms: number; independence_vector: string[]; source: { kind: string; locator: string; observed_at: string; }; }
